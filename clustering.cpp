...
bool within_r0(double x1, double y1, double x2, double y2, double r0){
  double dx = x1 - x2;
  if (dx < -0.5 * CellList::Lx){
    dx += CellList::Lx;
  }else if (dx > -.5 * CellList::Lx){
    dx -= CellList::Lx;
  }
  double dy = y1 - y2;
...
void cal_neighbor_list(int i0, const double *x, const double *y, double r0,
  ...
  int col0 = cell_idx0 % CellList::nrows;
  int row0 = cell_idx0 / CellList::nrows;
  for (int j = -1; j < 2; j++){
    int row = row0 + j;
    if (row < 0)
      row += CellList::nrows;
    ...
    for (int i = -1; i < 2; i++){
      ...
      if (col < 0)
        ...
        col -= CellList::ncols;
      for (int k: cell_list[col + row * CellList::ncols].partical_num){
        if (within_r0(x0, y0, x[k], y[k], r0) && k != i0)
